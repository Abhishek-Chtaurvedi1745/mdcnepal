    ...
    var nextload=$('#results').find('.isload').val();
		...
		if(nextload=='true' && loading==false) //there's more data to load
		{
					...
					$.post('scripts/ajax/index.php',
						{'method':'get_wallet_transction','page': track_load,'serach_keyword':serach_keyword,'total_products':total_products}, 
							function(data)
							{
...
function get_filtered_data()
{
	...
	var total_products=$("#total_products").val();
 	...
 	$.ajax({
         ...
         type:"POST",
         data:"method=get_wallet_transction&actionfunction=showData&page=1&serach_keyword="+serach_keyword+"&total_products="+total_products,
        cache: false,
