	...
	class model extends Singleton{
		...
		public function join_table($table, $join_type="left", $fields=array(), $keys=array(), $join_condition=""){ 			
			...
			if($model_class == NULL){
				...
				if(method_exists($obj_join_model, $model_class)){
					$obj_join_model->$model_class();
				}
			...
			if($primary_tbl_name != $this->tablename){
				...
				if($model_class == NULL){
					...
					if(method_exists($obj_join_model_primary, $model_class)){
						$obj_join_model_primary->$model_class();
					}
