...
{
	//Edit brand
	$obj_brand = $app->load_model("brand");
  ...
  <div class="modal-dialog modal-dialog-centered modal-lg" role="document">
    <div class="modal-content tx-14">
      <div class="modal-header">
        <h6 class="modal-title" id="exampleModalLabel2">Brand Form</h6>
        <button type="button" class="close" data-dismiss="modal" aria-label="Close"> <span aria-hidden="true">&times;</span> </button>
      </div>
      <form method="post" name="brand_form" id="brand_form"  data-parsley-validate>
      ...
      <div class="form-group col-md-6">
        ...
        <div class="fileupload fileupload-new" data-provides="fileupload">
            ...
            <a href="#" class="btn btn-xs fileupload-exists btn-white" data-dismiss="fileupload">Remove</a>
             ...
             <?php if($id!='' && $image!=''){ ?>
               <button id="btn_<?=$id?>_image" class="btn btn-xs btn-outline-danger" onclick="remove_image('<?=$id?>','brand','image','brand')" type="button">Remove</button>
              <?php }?>
            ...
            <div class="form-group col-md-6">
              <label for="inputEmail4">Sort Order</label>
           		<? $app->htmlBuilder->buildTag("select", array("class"=>"form-control ","selected"=>$sort_order, "values"=>$app->utility->sort_order('brand'),"required"=>""), "sort_order") ;?>
            <br />
