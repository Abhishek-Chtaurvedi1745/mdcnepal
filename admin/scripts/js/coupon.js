...
$(document).ready(function(){
	$('#table_coupon').DataTable({
		...
		'serverMethod': 'post',
		'ajax': {
			'url':'scripts/ajax/index.php?method=coupon&actionType=coupon_list',
		},
		'columns': [
		 	{ "data": "checkbox" },
		  	{ "data": "id" },
...
$(document).on("click",".coupon_delete_onclick", function ()
{
            ...
            }, function (r)
			{
				if(r == true)
				{
					$.ajax(
					{
						  ...
						  data: "method=coupon&actionType=couponDelete&getid="+getid,
						  success: function(responseData)
						  {
								  if(responseData.RESULT==0)
								  {
									var oTable = $('#table_coupon').dataTable( );
...
function mulitple_coupon_select()
{
			var chk_vals=[];
						...
						if(r == true)
						  {
							  ...
							  dataType: 'json',
							  url: "scripts/ajax/index.php",
							  data: "method=coupon&actionType=couponMultiDelete&ids="+ids,
							  success: function(responseData){
								  if(responseData.RESULT==0)
								  {
