...
$(document).ready(function(){
	...
	$('#table_product').DataTable({
		...
		"columnDefs": [ { 'targets': [0,2,4,6,8],"orderable": false } ],
		...
		'ajax': {
			...
			'url':'scripts/ajax/index.php?method=product&actionType=product_list&current_status='+current_status,
...

			{
...

				{
...

					{
						  ...
						  data: "method=product&actionType=productDelete&getid="+getid,
...

			{
...

				{
...

					{
						  ...
						  data: "method=product&actionType=productpriceDelete&getid="+getid,
...
function change_product_status(id,value)

{
				...
				$.ajax(

					{
						  ...
						  data: "method=product&actionType=product_status_update&getid="+id+"&value="+value,
...
function change_status_1(id, table_name, current_status){
	$('#status_best_'+id).removeAttr('onclick');
	$.ajax(
		{
			type: "POST",
			url: "scripts/ajax/index.php",
			data: "method=change_status_best&id="+id+"&table_name="+table_name+"&current_status="+current_status,
			success: function(msg)
			{
				if(msg=='OK')
				{
					var status = (current_status=='Yes')?'assets/img/status/No.png':'assets/img/status/Yes.png';
					$('#status_best_'+id).attr('src',status);
					$('#status_best_'+id).unbind('click');
					$('#status_best_'+id).click(function(){change_status_1(id, table_name, current_status=='Yes'?'No':'Yes')});
					 swal({
						 title:"Successfully Updated.",
						 type:"success",
              			 timer: 1500
...
function change_status_2(id, table_name, current_status){
	$('#status_new_'+id).removeAttr('onclick');
	$.ajax(
		{
			type: "POST",
			url: "scripts/ajax/index.php",
			data: "method=change_status_new&id="+id+"&table_name="+table_name+"&current_status="+current_status,
			success: function(msg)
			{
				if(msg=='OK')
				{
					var status = (current_status=='Yes')?'assets/img/status/No.png':'assets/img/status/Yes.png';
					$('#status_new_'+id).attr('src',status);
					$('#status_new_'+id).unbind('click');
					$('#status_new_'+id).click(function(){change_status_2(id, table_name, current_status=='Yes'?'No':'Yes')});
					 swal({
						 title:"Successfully Updated.",
						 type:"success",
              			 timer: 1500
