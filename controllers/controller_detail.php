	...
	function onload() {
		...
		if($slug!='')
		{
			...
			if($item_type==1)
			{
				...
				$obj_model_packages->join_table("item_description", "left", array(), array("item_id"=>"item_id"));
				$rs_package_data = $obj_model_packages->execute("SELECT",false,"","item_package_data.item_id='".$rs_data[0]['id']."'","");
				
				// $obj_model_item_description = $this->app->load_model("item_description");
				// $rs_item_description = $obj_model_item_description->execute("SELECT",false,"","item_id='".$rs_data[0]['id']."'","");
				// print_r($rs_item_description); exit;
			}
			...
			$meta_schema = str_replace($default_string, $new_string,$rs_data[0]['item_other_data_meta_schema']);
