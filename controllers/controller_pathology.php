...
class _pathology extends controller{
	...
	function onload()
	{
		...
		if($city=='' && $desc=='blood-test')
		{
			$this->app->redirect(SERVER_ROOT."/lab/blood-test/".$_SESSION['citySlug']);
		}
		...
		$rs_banner=$obj_model_banner->execute("SELECT",false,"","status='Active' and show_page='pathology' and (FIND_IN_SET ('".$_SESSION['cityID']."',banner.city_ids) or city_ids='')","sort_id ASC");
